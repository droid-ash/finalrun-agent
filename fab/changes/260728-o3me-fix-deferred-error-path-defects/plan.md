...
- *Consolidation (not deletion) candidate, deferred by design*: the push-and-cap statement pair is now written four times (`AndroidRecordingProvider.ts:246-250,254-258`; `DeviceDiscoveryService.ts:618-623,624-629`). R4's measured-diff test is applied at accumulator-body granularity and correctly rejects a shared helper there, but a `pushBounded(buffer, chunk)` leaf beside the constant would collapse all four. Left for a future change rather than forced here.
