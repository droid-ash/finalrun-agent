...
`packages/cloud-core` submits a run to the cloud: `submit.ts` reads the environment contract, zips the spec files and uploads, and `appBundle.ts` resolves the user's `--app` path into something uploadable, zipping a `.app` directory into `os.tmpdir()` when one is given. Both create temp artifacts, and each artifact's release is scoped to the acquisition that created it.

## Requirements

### Requirement: `FINALRUN_SUBMIT_TIMEOUT_MS` accepts a positive integer value

`parseSubmitTimeoutMs` MUST accept only a parsed value that is both integral and positive (`!Number.isInteger(parsed) || parsed <= 0` throws `Invalid FINALRUN_SUBMIT_TIMEOUT_MS=…: must be a positive integer (milliseconds).`), so the guard and the message state one rule. The check tests the parsed **value**, never the literal's spelling: `'1e3'` and `'0x10'` are accepted as `1000` and `16`, and `'1.5'` is rejected. A string-format regex MUST NOT be introduced — it would reject integral spellings the message never promises to reject. `Number.isInteger` subsumes the finiteness test (`NaN` and `±Infinity` are not integers), so there is no redundant clause. An unset or empty variable falls back to the caller's default. Both directions are pinned by tests: the rejection of the fractional value and the acceptance of the exponent and hex spellings.
