...
| [common](common/index.md) | `packages/common` — the base of the dependency graph, imported by every other package: the UI-hierarchy parse contract planner and grounder prompts are built from (two non-equivalent parse paths, presence-based alias resolution, two bounds forms, unvalidated casts), the `${variables.*}`/`${secrets.*}` placeholder contract in both directions (with the bounds and residuals of secret redaction), and the `CliEnv` environment-loading contract (dotenv/OS precedence, falsy-value traps) |
