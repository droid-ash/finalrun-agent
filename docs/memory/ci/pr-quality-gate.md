...
- **Scheduling**: `ci.yml` carries **no `concurrency` key at all**, so no run is ever cancelled
  ...
  default loses the middle run of three rapid pushes. `concurrency.queue: max` would keep the
  pending runs at a cost the Design Decisions below record as a rejected trade, not an
  impossibility. "Every push" itself comes from the `synchronize` activity type,
...
trigger, and a dispatched run would satisfy the required check on a **weaker** signal than the
`pull_request` run it stands in for (mechanism in the rejected-alternative record below). The
working recoveries are to push (an empty commit suffices) or to
...
**Test files are exempt from `max-lines-per-function`, and from that rule only.** A later flat-config object sets it `off` for `['**/test/**/*.{ts,tsx}', '**/*.test.{ts,tsx}', '**/*.spec.{ts,tsx}']` — the glob set MUST cover both layouts the repo uses (per-subfolder `test/` directories *and* the `.test.ts` suffix; either pattern alone under-matches), and lists `.spec` because `fab/project/config.yaml` `test_paths` does. `complexity` and `max-depth` still apply to test files, so test-file `complexity` findings stand rather than being swept up (decision below).

`@typescript-eslint/no-unused-vars`, `max-depth`, and `prefer-const` are clear tree-wide and MUST stay clear — a new violation of any of the three is a regression introduced by the PR, not inherited debt. Every remaining warning is `max-lines-per-function` or `complexity`: **42 — 20 source `max-lines-per-function`, 16 source `complexity`, 6 test-file `complexity`**, with `packages/common`'s sources at zero. Those two rules are what still blocks promoting the set from `warn` to `error`. Part of the `complexity` count is an artifact of the rule counting each `??` as a branch, which trips data-shaped mappers that carry no control flow — [/common/hierarchy.md](/common/hierarchy.md) records the property and what clearing it honestly costs.
...
### Test files are exempt from function length, never from branching
**Decision**: `max-lines-per-function` is `off` on the test globs; `complexity` and `max-depth` stay on for tests, and the surviving test-file `complexity` findings stay visible in `npm run lint` output rather than being exempted with the length rule.
**Why**: Function length is a claim about **shape** — a production function accumulating responsibilities past what a reader holds at once — and a test body is not that shape: its length is almost all *arrange* (fixtures, stubs, a hand-built harness), so hoisting it into helpers moves the setup away from the assertions it explains. The three longest functions in the repo are test bodies (234, 157, 153 lines), each a case where inlined setup is the clearer choice. Branching inside a test is a different claim: a test that makes decisions can pass for the wrong reason, its own branches unverified — so `complexity` and `max-depth` keep earning their keep there. The length rule also dominated the number the `warn` → `error` promotion is gated on — 29 of the 78 warnings it then reported were test-file lengths, 37% of the count — and a metric mixing "must fix" with "will never fix" cannot be driven to zero, let alone promoted to `error`.
**Rejected**: exempting test files from all three code-quality rules — the reading of "exempt tests" that silently sweeps up the test-file `complexity` findings, each a live signal about a test that can pass for the wrong reason. Exempting length judges shape; exempting branching would judge correctness, and those are not the same call.
...
**Decision**: A package with no dependency-injection seam is characterized by stubbing only the process globals it genuinely crosses — `globalThis.fetch`, `console.log`, `Date.now`, each restored in a `finally` — and using real temp workspaces (`fs.mkdtempSync`) for every filesystem effect, so the actual zip, upload-blob and cleanup paths execute. No seam is added to make the code reachable. Behavior that stays unreachable under that constraint is recorded as an open gap rather than forced: in `cloud-core` the spinner strings behind `submitRun`'s dynamic `await import('ora')` are unpinned for exactly this reason. Those same globals make temp-artifact uniqueness reachable, pinned by `submit.test.ts`'s tmpdir-diff on the stable `finalrun-(cloud|app)-` prefix and never on the random component (260726-pqux).
...
`cancel-in-progress: false` + `queue: max` — valid, and it would also keep every pending run (FIFO,
up to 100 pending; a validation error when combined with `cancel-in-progress: true`), rejected as a
trade-off, not an impossibility: it serialises per-push
...
`pull_request` run checks out — a weaker signal — and for a fork PR it would run in the fork, not
